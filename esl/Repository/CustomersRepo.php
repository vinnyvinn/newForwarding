...
class CustomersRepo
{
    ...
    public function searchCustomers($searchItem, $table, $currency=1)
    {
        if ($table == 'Client') {
            if(!empty($currency)){
                $currency = $currency=='KES'?0:1;
                return Customer::where(function ($q) use($searchItem) {
                    $q->where('Name', 'like', '%' . $searchItem . '%')->orWhere('Contact_Person', 'like', '%' . $searchItem . '%');
                })->where('iCurrencyID',$currency)
                    ->with('currency')
                    ->orderBy('iCurrencyID')
                    ->get(['DCLink', 'Name', 'Account', 'Contact_Person', 'Telephone','EMail','iCurrencyID']);
            }

            return Customer::where('Name', 'like', '%' . $searchItem . '%')
